  ...
  function cargarDatos() {
    fetch("https://navigationasistance-backend-1.onrender.com/usuariocapuntoscontrol/listarGrupo/cavent")
      ...
      .catch(err => {
        ...
        alert("Los competidores no han podido ser cargados. Controlar el backend.");
      });
  }
  ...
  $('#navegantes').on('click', '.eliminar-btn', function () {
    ...
    if (confirm(`¿Está seguro que desea eliminar los checkpoints del usuario ${usuarioId}?`)) {
      fetch(`https://navigationasistance-backend-1.onrender.com/usuariocapuntoscontrol/eliminarUsuariocaPuntos/${encodeURIComponent(usuarioId)}`, {
        ...
        .then(() => {
          alert("Los puntos de control del usuario fueron exitosamente eliminados.");
          cargarDatos();
        })
        .catch(err => {
          console.error("Error eliminando usuario:", err);
          alert("No se pudo borrar los puntos de control. Controlar el backend.");
        });
    }
  });
});
