...
document.addEventListener("DOMContentLoaded", () => {
  // 🔍 Obtener parámetro 'usuario' de la URL
  const params = new URLSearchParams(window.location.search);
  const usuarioDesdeURL = params.get('usuario');
  ...
  const api_url = new URL("https://navigationasistance-backend-1.onrender.com/nadadorposicion/listarActivosPorGrupo/cavent");
  let firstTime = true;
  let usuarioAutoActivado = false; // Flag para evitar activar múltiples veces
  ...
  async function getSwimmer() {
    try {
      ...
      for (let nadador of data) {
        ...
        // 🎯 Activar automáticamente si el usuario coincide con el de la URL
        if (!usuarioAutoActivado && usuarioDesdeURL && String(usuarioid) === String(usuarioDesdeURL)) {
          console.log(`✅ Activando traza automática para usuario: ${usuarioid}`);
          activarTrazaParaUsuario(usuarioid, position, nombre);
          usuarioAutoActivado = true;
        }

        // Seguir al nadador solo si hay traza activa y es el usuario seleccionado
        if (trazaActiva && usuarioid === naveganteSeleccionadoId) {
