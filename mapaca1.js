...
function crearIconoCompetidor(colorIndex = 0) {
  ...
  return L.divIcon({
    className: 'competidor-icon',
    html: `
      <div style="
        width: 34px;
        height: 50px;
        background-image: url('img/aventurero.png');
        background-size: contain;
        background-repeat: no-repeat;
        background-position: center;
        filter: hue-rotate(${colorIndex * 36}deg) saturate(1.2);
        position: relative;
      ">
        <div style="
          position: absolute;
  ...
  try {
    ...
    nadadores.forEach(n => {
      ...
      if (n.emergency === true) {
        ...
        icono = crearIconoCompetidor(n.usuarioid % 10);
      }
