...
async function cargarNavegantesVinculados() {
  try {
    ...
    // 🔑 set de IDs activos (string)
    const idsActivos = new Set();

    for (const n of nadadores) {
      const userId = String(n.usuarioid);   // 🔒 NORMALIZACIÓN CLAVE
      ...
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
        console.warn(`❌ Coordenadas inválidas para ${userId}`, n);
        continue;
      }
