...
const USUARIO_ID_URL = obtenerUsuarioIdDesdeUrl();
...
function obtenerUsuarioIdDesdeUrl() {
  ...
  // Soporta: maparepew.html?111114
  const valorDirecto = search.substring(1).trim();

  // Opcionalmente soporta también: ?usuarioid=111114
  const params = new URLSearchParams(search);
  const valorParam = params.get("usuarioid") || params.get("usuario");

  return valorParam || valorDirecto || null;
}
...
async function cargarNavegantesVinculados() {
  try {
    ...
    let nadadores = await response.json();

    if (USUARIO_ID_URL) {
      try {
        await fetch(
          `https://navigationasistance-backend-1.onrender.com/usuarios/listarId/${USUARIO_ID_URL}`
        );

        nadadores = nadadores.filter(
          (n) => String(n.usuarioid) === String(USUARIO_ID_URL)
        );
      } catch (e) {
        console.warn("No se pudo validar usuario por URL:", e);
        nadadores = [];
      }
    }
